...
autoEncoder_CNN.compile(loss=[AE_loss,Classifier_loss],optimizer = 'sgd',loss_weights=[1,0])
...
def generate_data(img_paths_list,label_list,total_image_num,batch_size,w,h,max_label,train_set):
	...
	while True:
		...
		yield (image_batch_input, [image_batch_output,label_batch]) ##(input, output)



# # checkpoint
filepath="models/AE_CNN_model_v5_refOnly_weights.{epoch:02d}-{val_conv2d_transpose_4_loss:.2f}.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_conv2d_transpose_4_loss', verbose=1, save_best_only=True, mode='min',save_weights_only=False) ### save model based on classification loss 
...
# print ('val_loss',MODEL.history['val_loss'])
# print ('train_loss',MODEL.history['loss'])

val_loss=MODEL.history['val_loss']
val_AE_loss=MODEL.history['val_conv2d_transpose_4_loss']
val_class_loss=MODEL.history['val_dense_2_loss']


train_loss=MODEL.history['loss']
train_AE_loss=MODEL.history['conv2d_transpose_4_loss']
train_class_loss=MODEL.history['dense_2_loss']

loss_file=open('models/loss.txt','a')
for i in np.arange(len(val_loss)): 
	loss_file.write(str(val_loss[i])+' '+str(val_AE_loss[i])+' '+str(val_class_loss[i])+' '+str(train_loss[i])+' '+str(train_AE_loss[i])+' '+str(train_class_loss[i])+'\n')
...
autoEncoder_CNN.save_weights("models/AE_CNN_model_v5_refOnly_weights.h5")
