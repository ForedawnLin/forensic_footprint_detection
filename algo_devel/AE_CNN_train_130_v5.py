...
def load_data_train(main_path,index_path,label_path):
	### use reference img only ###
	pic_fmt='.jpg'  ## picture format 
	imagePaths_list=[]  ## init image paths
	FileID_index=open(index_path,'r')
	for indice in FileID_index:
...
###### reference image only #######
	...
	r_num=[] 
	i=0;
	for each_index in indice:
		if 'r' in each_index:
			index_r.append(each_index)
			r_num.append(i)
		i+=1
...
####### ends ######################
	...
	#print (label)
	imagePaths_list=[main_path+index_r[i]+pic_fmt for i in np.arange(len(index_r))]
	#label=[int(label[i])-1 for i in np.arange(len(label))]  ## -1 b/c zero index 
	label=[int(label[i])-1 for i in r_num]  ## reference image only  
	return imagePaths_list,label 
...
a=train_random_paths[0].split('/')

print ('train_path',a[0]+'/'+a[1]+'/'+a[2]+'/'+a[3]+'/'+a[4]+'/'+a[5]+'/'+a[6])
...
def generate_data(img_paths_list,label_list,total_image_num,batch_size,w,h,max_label):
	...
	while True:
		...
		for index in np.arange(batch_size):
			...
			img_path_no_noise=img_path_list[0]+'/'+img_path_list[1]+'/'+img_path_list[2]+'/'+img_path_list[3]+'/'+img_path_list[4]+'/'+'train_noise'+'/'+img_path_list[6]
		...
		yield (image_batch, [image_noNoise_batch,label_batch]) ##(input, output)


# # checkpoint
filepath="models/AE_CNN_model_v5_130_weights.{epoch:02d}-{val_dense_2_loss:.2f}.hdf5"
...
MODEL=autoEncoder_CNN.fit_generator(generator=generate_data(train_random_paths,train_random_label,train_num,batch_size,resize_w,resize_h,max_label),
